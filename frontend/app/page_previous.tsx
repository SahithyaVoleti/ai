...
function HomeContent() {
  ...
  useEffect(() => {
    ...
    if (silenceCountdown !== null && silenceCountdown > 0) {
      ...
      if (recognitionRef.current && isListening) {
        console.log("GÅ¦n+Å Timer reached 0. Stopping recording.");
  ...
  // Web Audio API lip-sync GÇö direct DOM refs to avoid 60fps React re-renders
  ...
  const handleInactivity = async () => {
    ...
    if (!hasRepeatedRef.current) {
      console.log("=ƒòÆ Inactivity (1m): Repeating question");
      ...
      console.log("=ƒòÆ Extended inactivity (1m after repeat): Skipping question");
  ...
  useEffect(() => {
    ...
    console.log("=ƒô¥ Current Transcript State:", { transcript, interimTranscript });
  }, [transcript, interimTranscript]);
      ...
      if (analyserRef.current) {
        ...
        if (stage === 'interview' && !isSpeakingRef.current) {
          if (currentLevel > 15) {
            ...
            if (isSilenced) {
              console.log("=ƒùún+Å Speech resumed - resetting timer");
              setIsSilenced(false);
              setSilenceCountdown(null);
            }
          } else {
            ...
            if (silenceDuration > 2000 && !isSilenced) { // 2s silence
              console.log("GÅ+n+Å Silence detected - starting 30s countdown");
              setIsSilenced(true);
        ...
        try {
          ...
          if (recorder.state === 'inactive') {
            try {
              ...
              console.warn("GÜán+Å MediaRecorder chunked start failed, trying basic start...");
              if (recorder.state === 'inactive') {
                 try { recorder.start(); } catch (err2) { console.error("G¥î MediaRecorder failed to start:", err2); }
              }
            }
            mediaRecorderRef.current = recorder;
          }
        } catch (recorderErr) {
          console.error("G¥î MediaRecorder failed:", recorderErr);
        }
      }
    } catch (e: any) {
  ...
  const syncCameraToElement = (el: HTMLVideoElement | null) => {
    ...
    if (el.srcObject !== s) {
      el.srcObject = s;
      console.log("=ƒÄÑ Camera synced to element:", el.id || "unnamed");
    }
    if (el.paused) {
      el.play().catch(e => {
        if (e.name !== "AbortError" && e.name !== "NotAllowedError") {
          console.warn("=ƒÄÑ Video play delayed/failed:", e.name);
        }
      });
    }
  };
  ...
  useEffect(() => {
    ...
    const init = async () => {
      if (isCameraNeeded) {
        ...
        if (isStreamActive) {
          console.log("GÖ+n+Å Reusing existing camera stream");
        ...
        try {
          console.log(`=ƒÄÑ Initializing camera for stage: ${stage}`);

          if (!navigator?.mediaDevices) {
            setFeedback("GÜán+Å Camera API not supported or insecure connection (use HTTPS or localhost).");
            return;
          }
          ...
          try {
            ...
            console.warn("GÜán+Å Balanced camera failed GÇö retrying with basic constraints");
            stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
          }

          streamRef.current = stream;
          setCurrentStream(stream);
          // Global backup GÇö survives React re-renders and stage transitions
          ...
          setFeedback('');
          console.log("G£à Camera stream active");

        } catch (e: any) {
          console.warn("=ƒô+ Media Error:", e.name, e.message);
          if (e.name === 'NotAllowedError' || e.name === 'PermissionDeniedError') {
            setFeedback("GÜán+Å Camera blocked! Click the =ƒöÆ lock icon in your browser address bar GåÆ Allow Camera & Microphone GåÆ then refresh.");
          } else if (e.name === 'NotReadableError' || e.name === 'TrackStartError') {
            setFeedback("GÜán+Å Camera in use by another app (Zoom, Teams). Close them and refresh.");
          } else if (e.name === 'NotFoundError' || e.name === 'DevicesNotFoundError') {
            setFeedback("GÜán+Å No camera found. Please connect a webcam and refresh.");
          } else {
            setFeedback(`GÜán+Å Camera error (${e.name}). Please refresh the page.`);
          }
        }
      } else {
        if (streamRef.current) {
          console.log("=ƒ¢æ Stopping camera stream (not needed for this stage)");
    ...
    const cameraWatchdog = setInterval(() => {
      if (isCameraNeeded) {
        ...
        if (tracksDead) {
          console.warn("=ƒô+ Watchdog: Stream dead. Auto-repairing...");
          init();
        } else {
  ...
  const restartCamera = async () => {
    console.log("=ƒöä Force-restarting camera hardware...");
    ...
    try {
      ...
      console.log("G£à Camera restarted successfully");
    } catch (e: any) {
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (SpeechRecognition) {
        ...
        console.log(`=ƒÄñ Recognition language set to: ${recognition.lang}`);

        // Heartbeat to detect if the engine is truly alive
        recognition.onsoundstart = () => {
          console.log("=ƒöè Engine detected actual sound...");
          lastResultTimeRef.current = Date.now();
        };
        ...
        recognition.onresult = (event: any) => {
          ...
          if (cleanFinalAdd) {
            console.log("=ƒÄñ Final committed phrase:", cleanFinalAdd);
        ...
        recognition.onstart = () => {
          ...
          console.log("=ƒÄñ Mic is ACTIVE / Listening");
        };
        ...
        recognition.onend = () => {
          setIsListening(false);
          recognitionActiveRef.current = false;
          console.log("=ƒÄñ Mic session closed.");
          ...
          if (!isResettingRef.current && recognition === recognitionInstanceRef.current && stage === 'interview' && !isSpeakingRef.current && !isManualStopRef.current && hasStartedRef.current) {
            console.log("=ƒöä Auto-repairing mic session...");
            setTimeout(() => {
    ...
    const watchdog = setInterval(() => {
      ...
      if (hasStartedRef.current && isStillInInterview && !shouldMicBeOff) {
        ...
        if (isStalled || isMissedSound) {
          console.log(isMissedSound ? "=ƒöÑ Mic missed sounds (Low Level). Force Restarting..." : "=ƒöä Voice engine stalled. Restarting...");
          try {
            recognitionActiveRef.current = false;
            isStartingRef.current = false;
            recognition?.abort();
        ...
        if (!recognitionActiveRef.current && !isStartingRef.current) {
          console.log("=ƒ¢án+Å Watchdog: Resuming mic listeners...");
  ...
  const handleSubmitAnswer = async () => {
    ...
    if (mediaRecorderRef.current && mediaRecorderRef.current.state !== 'inactive') {
      ...
      const whisperTranscript = await new Promise<string>((resolve) => {
        recorder.onstop = async () => {
          ...
          try {
            console.log("GÅ¦ Sending audio to Whisper for high-accuracy processing...");
            ...
            if (transData.status === 'success' && transData.transcript) {
              console.log("G£¿ Whisper Result:", transData.transcript);
    ...
    if (!finalUserAns || finalUserAns.trim().length === 0) {
      console.log("GÜán+Å No answer detected. Sending '-1' as requested.");
      finalUserAns = "-1";
    }
  ...
  const handleRunCode = async () => {
    ...
    if (!currentProblem || !codeAnswer.trim()) {
      setFeedback("GÜán+Å Please write some code first.");
      return;
    }

    setIsRunningCode(true);
    setCodingOutput("GÅ¦ Running test cases...\n");
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      try {
        ...
        if (actual === expected) {
          passed++;
          logs.push(`G£à Test Case ${i + 1} Passed`);
        } else {
          logs.push(`G¥î Test Case ${i + 1} Failed (Expected: ${expected}, Got: ${actual})`);
        }
      } catch (error) {
        logs.push(`G¥î Test Case ${i + 1} Error: Connection failed`);
      }
    }
  ...
  const handleSubmitCode = async () => {
    if (showFullscreenWarnRef.current || showTabSwitchWarnRef.current) return;
    if (!codeAnswer.trim()) {
      setFeedback("GÜán+Å Please write your solution before submitting.");
      return;
    }
    ...
    try {
      ...
      setFeedback("GÜán+Å Submission failed. Continuing...");
      handlePhaseProgress();
    } finally {
      setLoading(false);
    }
  ...
  const handleDownloadReport = (isManual = false) => {
    ...
    console.log(isManual ? "=ƒôÑ Manual report request..." : "=ƒôÑ Auto-downloading report...");
    ...
    speak(reason, async () => {
      ...
      try {
        ...
        if (data.status === "success") {
          console.log("G£à Final result received:", data);
          setReportData(data);
      ...
      try {
        ...
        if (warning.toLowerCase().includes("movement") || warning.toLowerCase().includes("down")) {
          ...
          if (lookingDownRef.current > 4) {
            setFeedback("GÜán+Å Please maintain focus on the screen.");
  ...
  useEffect(() => {
    ...
    if (isSpeaking) {
      ...
      // Agent done speaking GÇö start listening
  ...
  const speak = (text: string, onComplete?: () => void, bypassDeferral = false) => {
    ...
    */
    ...
    if (!bypassDeferral && (showFullscreenWarnRef.current || showTabSwitchWarnRef.current || isNotInFullscreen)) {
      ...
      console.log("GÅ¦ Speak deferred (Visibility Violation)");
    ...
    const playFallback = async () => {
      ...
      console.log("=ƒöè Fetching OpenAI TTS for:", text.slice(0, 50));
      ...
      try {
        ...
        audio.onerror = (e) => {
          console.warn("GÜán+Å Audio Playback Error:", e);
          stopLipSync();
          setIsSpeaking(false);
          if (onComplete) onComplete();
        };
        ...
        console.warn("GÜán+Å TTS fetch failed (using local fallback):", err.message);
  ...
  const handleBeginInterview = async () => {
    ...
    try {
      ...
      setFeedback("G¥î Connection error. Please ensure backend is running.");
      console.error(e);
    } finally {
      setLoading(false);
    }
  ...
  const captureAndVerify = async () => {
    ...
    if (!video || video.readyState < 2 || video.videoWidth === 0) {
      setVerifyStatus("=ƒö¦ Camera not ready. Please wait for video feed or restart camera.");
      return;
    }

    // STRICT face identification enforcement
    if (!proctorStatus.face) {
      setVerifyStatus("=ƒö¦ Please position your face clearly in the camera frame to proceed.");
    ...
    try {
      ...
      if (data.status === 'success') {
        setVerifyStatus("G£à " + (data.message || "Identity & Eyes Verified Successfully!"));
        ...
        setVerifyStatus("G¥î " + (data.message || "Verification failed."));
      ...
      setVerifyStatus("G¥î Connection error. Identity verification is required to proceed.");
      setVerifying(false);
    }
  };
  ...
  const confirmEndInterview = async () => {
    ...
    try {
      ...
      if (finalUserAns && finalUserAns.length > 5 && (stage === 'interview' || stage === 'code')) {
        console.log("=ƒô¥ Submitting final partial answer before ending...");
        await fetch(getApiUrl("/api/interview/answer"), {
          method: "POST",
          headers: { 'Content-Type': 'application/json' },
              ...
              <div className="flex-1 flex flex-col bg-[#111827]">
                <div className="bg-[#1e293b]/50 px-4 py-2 border-b border-slate-800 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    >
                      {isRunningCode ? <span className="animate-spin text-lg">GÅ¦</span> : <Play size={14} />}
                      Run Code
                    </button>
                  </div>
                </div>
          ...
          <main className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-[var(--background)] overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-5xl h-full flex flex-col md:flex-row items-center justify-center gap-12 p-6 md:p-12">
              ...
              <div className="flex-1 flex flex-col items-center w-full max-w-lg">
                ...
                {/* Status Message */}
                {verifyStatus && (
                  <div className={`mt-6 px-6 py-3 rounded-2xl font-bold text-sm transition-all animate-in fade-in slide-in-from-bottom-4 shadow-lg flex items-center gap-3 ${verifyStatus.includes('G£à') ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}>
                    {verifyStatus.includes('G£à') ? <CheckCircle size={18} /> : <AlertCircle size={18} />}
                    {verifyStatus}
                  </div>
                )}
              ...
              <div className={`flex-[2.5] flex flex-col relative rounded-[3rem] overflow-hidden bg-slate-900 shadow-2xl border-4 transition-all duration-500 ${isSpeaking ? 'border-blue-500 shadow-blue-500/20' : 'border-slate-800'}`}>
                ...
                <div className="absolute inset-0 flex items-center justify-center" style={{ background: 'radial-gradient(circle, #1e293b 0%, #0f172a 100%)' }}>
                  <div className="relative w-full h-full overflow-hidden shadow-2xl">
                    ...
                    {/* LIP SYNC BARS GÇö driven by Web Audio API analyser, directly updated via DOM ref */}
                    {true && (
                      <div
                      ...
                      >
                        {[0, 1, 2, 3, 4, 5, 6].map((i) => (
                          <div
                            ...
                            style={{
                              width: '8px',
                              height: '3px',        // starts flat GÇö RAF loop updates this
                      ...
                      <div className="absolute top-0 right-0 bottom-0 left-0 z-[100] flex flex-col items-end justify-end p-10 pointer-events-none animate-in fade-in duration-500">
                        ...
                        <div className="relative z-[101] pointer-events-auto flex flex-col items-end">
                          ...
                          <div className="mt-6 px-4 py-1 bg-white/5 rounded-full border border-white/10 text-[9px] font-black uppercase tracking-[0.3em] text-white/60 animate-pulse">
                            Silence Detected GÇö Recording Paused
                          </div>
                        </div>
                      </div>
                    )}
                ...
                <div className="absolute bottom-12 left-0 right-0 z-40 flex flex-col items-center">
                  ...
                  <div className="mt-[-30px] relative z-50 pointer-events-auto">
                    <button
                      onClick={() => {
                        console.log("=ƒ¢án+Å Manual Mic Reset Triggered");
                        try { recognitionRef.current?.abort(); } catch (e) { }
                        isStartingRef.current = false;
                        recognitionActiveRef.current = false;
                        isManualStopRef.current = false;
